...
def draw_hand_label(image, brect, handedness):
    if handedness is None:
        return image  # Skip drawing if handedness info is missing

    font = cv.FONT_HERSHEY_SIMPLEX
    font_scale = 0.7
    font_thickness = 2
    text_color = (255, 0, 0)  # Blue for Right Hand, Red for Left Hand
    ...
    # Draw the label near the bounding rectangle
    text_position = (brect[0], brect[1] - 10)
    cv.putText(image, handedness, text_position, font, font_scale, text_color, font_thickness, cv.LINE_AA)
